...
- **新增** 字体管理系统：完整的字体选择器、实时预览和重启提示功能。

**更新** 新增字体管理系统集成，提供完整的字体选择器功能，支持系统字体列表获取、实时预览和重启提示。

## 项目结构
设置视图位于主窗口的多标签页视图中，采用垂直导航标签页组织不同设置类别，各设置项通过 WinForms 控件绑定到全局配置对象，并在变更时持久化到 JSON 配置文件。新增的字体管理系统通过 FontManager 组件提供全局字体控制能力，在应用程序启动时进行初始化。
...
PRG["Program<br/>应用程序入口"]
MD["MacroDeck<br/>主应用程序"]
end
VT --> T1
VT --> T2
VT --> T3
...
- [MacroDeck.cs:161-162](file://src/MacroDeck/MacroDeck.cs#L161-L162)
...
- **新增** 字体管理器：提供全局字体控制、实时预览和重启提示功能，支持应用程序启动时的默认字体设置。
- **新增** 应用程序入口：在创建任何窗口之前设置应用程序默认字体，确保字体管理器的完整初始化。
...
**更新** 字体设置变更时，通过 FontManager 实时刷新界面字体，并显示重启提示。应用程序启动时通过 Program 和 MacroDeck 类分别设置默认字体和初始化字体管理器。
...
FM->>FM : 实时刷新所有窗体字体
SV->>SV : 显示重启提示
Note over PRG,MD : 应用程序启动时
PRG->>FM : SetDefaultFontEarly()
MD->>FM : Initialize()
FM->>FM : 设置全局字体配置
```
...
- **初始化流程**：应用程序启动时通过 Program.SetDefaultFontEarly 和 MacroDeck.Initialize 分别设置默认字体和初始化全局字体配置。
...
Program --> FontManager : "设置默认字体"
MacroDeck --> FontManager : "初始化字体"
```
...
- **新增** 字体管理：使用条件弱表缓存控件原始字体，支持幂等重算和实时刷新；字体检测使用 InstalledFontCollection，避免重复扫描；应用程序启动时提前设置默认字体，确保字体管理器的完整初始化。
- **新增** 应用程序入口优化：Program.SetDefaultFontEarly 在创建任何窗口之前设置默认字体，避免字体闪烁问题。
...
- **新增** 字体设置问题：确认字体已安装；检查字体列表加载；查看重启提示；某些元素需要重启才能完全生效；检查应用程序启动时的字体初始化是否正常。
- **新增** 字体初始化失败：检查配置文件中的字体设置；确认字体管理器的 Initialize 和 SetDefaultFontEarly 方法是否正确调用。
...
- [FontManager.cs:52-59](file://src/MacroDeck/Utils/FontManager.cs#L52-L59)
- [Program.cs:107-142](file://src/MacroDeck/Program.cs#L107-L142)
...
**更新** 新增的字体管理系统提供了完整的字体控制能力，支持系统字体列表获取、实时预览和重启提示，进一步提升了用户的界面定制体验。通过应用程序启动时的双重字体初始化机制，确保了字体管理器的完整性和稳定性。
...
- **新增** 字体：从系统 InstalledFontCollection 获取字体列表，支持实时预览和重启提示；配置包含字体族、字号和粗体三个维度。
