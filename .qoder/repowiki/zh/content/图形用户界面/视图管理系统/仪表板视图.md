...
- [QrCodeService.cs](file://src/MacroDeck/Services/QrCodeService.cs)
- [QuickConnectQrCodeData.cs](file://src/MacroDeck/DataTypes/QrCode/QuickConnectQrCodeData.cs)
</cite>

## 更新摘要
**变更内容**
- 更新网络信息面板章节，反映QR码显示区域从200x200扩大到240x240像素的改进
- 更新网络信息面板尺寸规格，从200x200像素提升到240x240像素
- 更新网络信息面板宽度从200像素增加到250像素的变更
- 新增QR码服务集成章节，说明QuickConnectQrCodeData数据结构
- 更新设备状态指示与实时信息章节，包含最新的QR码显示规格
...
DeckView --> NetInfo["网络信息面板<br/>二维码/接口/端口<br/>240x240像素QR码区域<br/>250像素面板宽度"]
...
- 设备状态与实时信息
  ...
  - **更新** QR码显示区域现已提升至240x240像素，提供更好的扫描体验和连接信息可见性。
  - **更新** 网络信息面板宽度从200像素增加到250像素，为QR码和网络信息提供更充足的显示空间。
- 数据绑定与事件驱动
...
- **新增** QR码服务层：QrCodeService 提供QuickConnectQrCodeData数据结构，支持动态生成连接二维码。
...
QrCodeService --> QuickConnectQrCodeData : "创建"
```
...
- [QrCodeService.cs:10-42](file://src/MacroDeck/Services/QrCodeService.cs#L10-L42)
...
### 设备状态指示与实时信息
- 二维码与网络信息
  - **更新** 顶部右侧显示二维码与网络接口列表及端口号，便于移动端快速连接与诊断。
  - **更新** QR码显示区域现已提升至240x240像素，提供更好的扫描体验和连接信息可见性。
  - **更新** 网络信息面板宽度从200像素增加到250像素，为QR码和网络信息提供更充足的显示空间。
  - **新增** QuickConnectQrCodeData数据结构支持动态生成连接二维码，包含实例名称、网络接口数组、端口、SSL状态和令牌。
- 窗口状态联动
...
- [QrCodeService.cs:14-41](file://src/MacroDeck/Services/QrCodeService.cs#L14-L41)
- [QuickConnectQrCodeData.cs:15-22](file://src/MacroDeck/DataTypes/QrCode/QuickConnectQrCodeData.cs#L15-L22)
...
- **更新** 网络信息面板自适应
  - QR码显示区域和网络信息面板尺寸可根据窗口大小自适应调整，提升不同分辨率下的显示效果。
...
- **更新** 改进的连接体验
  - QR码显示区域扩大至240x240像素，提升移动端扫描的准确性和便利性。
  - 网络信息面板宽度增加到250像素，为连接信息提供更充足的空间显示。
...
- **新增** QR码服务依赖
  - QrCodeService 提供QuickConnectQrCodeData数据结构，支持动态生成连接二维码。
...
QrCodeService --> QuickConnectQrCodeData
```
...
- [ButtonLabel.cs:6-61](file://src/MacroDeck/ActionButton/ButtonLabel.cs#L6-L61)
- [ProfileManager.cs:20-31](file://src/MacroDeck/Profiles/ProfileManager.cs#L20-L31)
- [QrCodeService.cs:12-12](file://src/MacroDeck/Services/QrCodeService.cs#L12-L12)
- [QuickConnectQrCodeData.cs:3-3](file://src/MacroDeck/DataTypes/QrCode/QuickConnectQrCodeData.cs#L3-L3)
...
- **更新** QR码渲染优化
  - QR码图像使用240x240像素高质量显示，同时通过内存流优化图像处理性能。
...
- **更新** QR码显示问题
  - 确认QR码显示区域已正确设置为240x240像素。
  - 检查网络信息面板宽度是否为250像素。
  - 如QR码无法显示，验证QrCodeService是否正常工作。
...
## 结论
仪表板视图为 Macro-Deck 提供了直观、可定制且高性能的按钮网格展示与交互体验。通过事件驱动的数据绑定、完善的上下文菜单与拖拽重排、以及针对渲染与内存的优化策略，它在保持良好用户体验的同时兼顾了扩展性与稳定性。

**更新** 最新改进包括QR码显示区域从200x200像素扩大到240x240像素，以及网络信息面板宽度从200像素增加到250像素，显著提升了连接信息的可见性和可用性。这些改进通过QuickConnectQrCodeData数据结构和QrCodeService服务得到支持，为用户提供更好的移动端连接体验。

建议在大型配置场景下关注变量更新与模板渲染的性能，合理使用缓存与异步处理以维持流畅体验。
...
- 配置项说明
  ...
  - 按钮背景：是否启用背景色覆盖。
- **更新** QR码配置
  - QR码显示区域：240x240像素，提供更好的扫描体验。
  - 网络信息面板宽度：250像素，为连接信息提供充足显示空间。
  - QuickConnectQrCodeData：包含实例名称、网络接口数组、端口、SSL状态和令牌的数据结构。
