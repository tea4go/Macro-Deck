...
## 项目结构
测试工程位于 tests/MacroDeck.Tests，采用 NUnit 作为测试框架，并通过 SDK 样式项目组织。测试工程引用主程序工程以进行集成测试与行为验证。**新增**：端到端测试脚本位于根目录，提供完整的应用级测试能力，包括自动化测试和本地更新脚本。
...
PS2["update-macrodeck-local.ps1"]
MD["自动化测试.md"]
end
TProj --> VM
TProj --> SC
...
- **新增**：自动化测试工作流
  - update-macrodeck-local.ps1 脚本集成 GitHub Actions 工作流
  - 支持触发构建、下载工件并更新本地安装
...
## 架构总览
测试架构围绕"测试工程引用主工程"的方式构建，测试用例直接调用被测类的公共或内部公开接口，确保在隔离环境中验证逻辑正确性与边界条件。**新增**：端到端测试脚本提供应用级集成测试，验证完整的服务栈，包括自动化测试工作流的集成。
...
### 自动化测试工作流（update-macrodeck-local.ps1）
- **新增**：集成 GitHub Actions 的自动化测试工作流
- 目标：触发构建、下载工件并自动更新本地 Macro Deck 安装
- 关键功能
  - 触发远程仓库的 build-local 工作流
  - 轮询构建状态直到完成
  - 下载构建工件并验证可执行文件
  - 安全地更新程序文件，保留用户数据目录
- 测试策略
...
```mermaid
flowchart TD
A["开始自动化测试"] --> B["检查 GitHub CLI"]
B --> C["触发 build-local 工作流"]
C --> D["轮询构建状态"]
D --> E["下载构建工件"]
E --> F["验证可执行文件"]
F --> G["停止现有进程"]
...
**图表来源**
- [update-macrodeck-local.ps1:43-72](file://update-macrodeck-local.ps1#L43-L72)
- [update-macrodeck-local.ps1:100-107](file://update-macrodeck-local.ps1#L100-L107)
...
- **新增**：端到端测试依赖
  ...
  - 需要访问应用程序安装目录和用户日志目录
  - update-macrodeck-local.ps1 依赖 GitHub CLI 进行工作流触发
...
GH["GitHub CLI"] --> Workflow["Actions 工作流"]
AutoTest["自动化测试"] --> GH
```
...
- 常见问题与定位
  ...
  - **新增**：自动化测试工作流失败：检查 GitHub CLI 是否正确安装、网络连接是否正常、工作流权限是否足够
- 日志与诊断
  - 利用日志库输出关键步骤与异常堆栈，便于快速定位问题
  - **新增**：端到端测试脚本提供详细的测试步骤输出和错误信息
  - **新增**：自动化测试脚本提供完整的构建状态和工件下载日志
- 修复与回归
  - 针对问题补充最小可复现用例，防止回归
  - 对修复进行小范围回归测试，确保影响面可控
  - **新增**：条件动作序列化缺陷修复后，立即运行回归测试确保问题不再出现
  - **新增**：自动化测试工作流集成后，定期运行以确保持续集成管道的稳定性
...
## 结论
本测试框架以 NUnit 为核心，结合中央包版本管理与统一编译配置，实现了对关键功能（字符串处理、加密解密、模板渲染、条件动作序列化）的稳定覆盖。**新增**：端到端冒烟测试脚本提供了完整的应用级集成测试能力，确保服务栈的完整性和可靠性。**新增**：自动化测试工作流集成了 GitHub Actions，实现了持续集成环境下的自动化测试和部署。通过参数化用例、文化固定、异常捕获和序列化测试等策略，提升了测试的可靠性与可维护性。建议在 CI 中引入覆盖率门禁与性能基准，持续提升质量与稳定性。
...
- 本地执行
  ...
  - **新增**：使用 update-macrodeck-local.ps1 脚本进行自动化测试工作流集成
- CI 集成
  - 在流水线中添加步骤执行测试与覆盖率收集
  - 将覆盖率报告上传至代码覆盖率平台，设置阈值门禁
  - **新增**：在 CI 中集成端到端测试，验证部署后的完整功能
  - **新增**：集成自动化测试工作流，实现持续集成和部署
...
- [update-macrodeck-local.ps1:11-21](file://update-macrodeck-local.ps1#L11-L21)