...
## 更新摘要
**所做的更改**
- 更新了中国语言包的翻译内容，包含62项新增翻译和22项删除
- 扩展了Strings.cs中的翻译键，新增32个翻译键
- 增强了网络配置、设备管理、动作配置、图标管理、扩展商店、模板编辑器、更新对话框等界面元素的完整本地化
- 完善了国际化系统的文档说明
...
**更新** 本次更新重点关注中国语言包的大幅扩展，新增62项翻译内容，涵盖网络配置、设备管理、动作配置、图标管理、扩展商店、模板编辑器、更新对话框等界面元素的完整本地化支持。
...
- GUI 初始设置与设置页的语言选择：src/MacroDeck/GUI/InitialSetupPages/SetupPage1.cs、src/MacroDeck/GUI/MainWindowViews/SettingsView.cs
- 项目文件对语言资源的嵌入声明：src/MacroDeck/MacroDeck.csproj

```mermaid
graph TB
subgraph "语言资源"
C["Chinese.json<br/>62项新增翻译"]
E["English.json"]
L["其他语言*.json"]
end
subgraph "语言管理"
LM["LanguageManager.cs"]
STR["Strings.cs<br/>32个新翻译键"]
end
subgraph "应用启动"
MD["MacroDeck.cs"]
...
**图表来源**
- [Chinese.json:1-365](file://src/MacroDeck/Resources/Languages/Chinese.json#L1-L365)
- [Strings.cs:1-443](file://src/MacroDeck/Language/Strings.cs#L1-L443)
...
**更新** 中国语言包现已包含365行翻译内容，涵盖网络配置、设备管理、动作配置、图标管理、扩展商店、模板编辑器、更新对话框等界面元素的完整本地化。
...
**更新** 中国语言包现已包含365行翻译内容，涵盖以下主要界面模块：
- 网络配置：网络适配器、IP地址、端口、配置信息等
- 设备管理：设备连接状态、设备设置、亮度控制等
- 动作配置：动作按钮、条件、触发器等
- 图标管理：图标包导入导出、图标质量设置等
- 扩展商店：扩展包管理、安装更新等
- 模板编辑器：模板引擎、变量绑定等
- 更新对话框：版本信息、下载进度、更新说明等
...
### 语言文件命名与嵌入
- 命名约定
  - 文件名即语言名称（如 English.json、Chinese.json）
...
- 系统语言自动检测
  - 通过 LanguageManager 的语言列表自动匹配可用语言
...
- [UpdateAvailableDialog.cs:26-36](file://src/MacroDeck/GUI/Dialogs/UpdateAvailableDialog.cs#L26-L36)
...
**更新** 本次更新显著增强了中国语言包的完整性，新增62项翻译内容，涵盖了应用的主要界面模块，为中文用户提供了更好的本地化体验。同时，Strings.cs 中新增的32个翻译键进一步完善了系统的国际化支持能力。
...
**更新** 针对中国语言包的翻译指导：
- 翻译内容涵盖网络配置、设备管理、动作配置、图标管理、扩展商店、模板编辑器、更新对话框等界面元素
- 注意保持术语一致性，如"设备"、"插件"、"图标包"等专业术语的统一翻译
- 关注占位符的正确处理，确保运行时替换功能正常工作
- 考虑中文的表达习惯，适当调整语序和用词，提升用户体验