...
- 新增 run_win.bat 批处理脚本的说明，提供便捷的 PowerShell 调用方式
...
**更新** 新增完整的 run_win.ps1 PowerShell 构建脚本，提供更强大的 Windows 构建自动化功能，包括 -Build 参数、配置参数和输出目录参数，以及自动生成的便携启动脚本。同时增强了 PowerShell 脚本的中文本地化支持，为中文开发者提供更好的开发体验。新增 run_win.bat 批处理脚本，提供便捷的 PowerShell 调用方式。
...
- **新增** 批处理启动：通过 run_win.bat 提供便捷的 PowerShell 调用方式，简化开发流程。
...
participant RunBat as "run_win.bat<br/>批处理启动脚本"
...
RunBat->>RunWin : 调用 PowerShell 脚本
```

**图表来源**
- [src/MacroDeck/Program.cs:12-35](file://src/MacroDeck/Program.cs#L12-L35)
- [run_win.ps1:53-94](file://run_win.ps1#L53-L94)
...
- [run_win.bat:2](file://run_win.bat#L2)
...
- **新增** 批处理脚本：run_win.bat 依赖 PowerShell 运行环境。
...
- **新增** 批处理优化：run_win.bat 提供便捷的脚本调用方式，简化开发流程。
...
- [run_win.ps1:77](file://run_win.ps1#L77)
- [.github/workflows/ci.yml:13-17](file://.github/workflows/ci.yml#L13-L17)
...
- **新增** 批处理问题：如 run_win.bat 无法调用 PowerShell，检查执行策略和路径设置。
...
- [run_win.ps1:54-57](file://run_win.ps1#L54-57)
...
- **防火墙规则**：自动配置防火墙规则，放行 TCP 8191 端口
- **主机地址更新**：自动更新配置文件中的主机地址为 Wi-Fi IP
...
# 指定自定义输出目录
.\run_win.ps1 -Build -Output "C:\Custom\Path"
...
- **防火墙配置**：自动创建 TCP 8191 端口的防火墙规则
- **主机地址更新**：自动检测 Wi-Fi IP 并更新配置文件
...
- **防火墙提示**：防火墙规则添加和更新使用中文提示
- **主机地址更新**：IP 地址更新和配置文件修改使用中文提示

### 本地构建更新器（update-macrodeck-local.ps1）
...
**新增** run_win.bat 是一个简单的批处理文件，提供便捷的 PowerShell 调用方式：

#### 功能特性
- **便捷调用**：通过批处理文件调用 run_win.ps1 PowerShell 脚本
- **参数传递**：完整传递命令行参数给 PowerShell 脚本
- **执行策略**：使用 Bypass 执行策略绕过 PowerShell 执行限制
...
# 传递参数给 PowerShell 脚本
.\run_win.bat -Build -Configuration Debug
```

#### 技术实现要点

- **PowerShell 调用**：通过 PowerShell -File 参数调用 run_win.ps1
- **执行策略**：使用 Bypass 执行策略确保脚本能够运行
- **参数传递**：使用 %* 传递所有命令行参数
- **路径解析**：使用 %~dp0 获取脚本所在目录的绝对路径
...
**重要说明**：根据更新原因，需要说明已删除的五个工作流文件的历史背景和替代方案。虽然当前仓库中可能不包含这些已删除的工作流文件，但它们曾经存在于项目中，为 CI/CD 流程提供了重要支撑。
...
- **新增** **批处理启动**：run_win.bat 提供便捷的脚本调用方式
- **新增** **全面中文本地化**：所有 PowerShell 脚本都支持中文界面和错误提示
...
- **新增** 批处理脚本：run_win.bat 提供便捷的 PowerShell 调用方式。
...
# 使用自定义参数
.\run_win.ps1 -Build -Configuration Debug -Output "C:\Custom\Path"
...
- **防火墙配置**：自动配置 TCP 8191 端口防火墙规则
- **主机地址更新**：自动检测 Wi-Fi IP 并更新配置文件
- **中文本地化**：所有构建状态、进度、结果都以中文显示

#### update-macrodeck-local.ps1 特色功能
**新增** update-macrodeck-local.ps1 提供了以下特色功能：
...
- **新增** 确保 PowerShell 执行策略允许运行本地脚本
- **新增** 以管理员身份运行以支持写入受保护目录
