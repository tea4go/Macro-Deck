...
**更新** 新增完整的 run_win.ps1 PowerShell 构建脚本，提供更强大的 Windows 构建自动化功能，包括 -Build 参数、配置参数和输出目录参数，以及自动生成的便携启动脚本。同时增强了 PowerShell 脚本的中文本地化支持，为中文开发者提供更好的开发体验。
...
- [run_win.ps1:27-29](file://run_win.ps1#L27-L29)
...
- [src/MacroDeck/Language/LanguageManager.cs:1-155](file://src/MacroDeck/Language/LanguageManager.cs#L1-L155)
...
- [Directory.Packages.props:1-35](file://Directory.Packages.props#L1-L35)
- [Directory.Build.props:1-11](file://Directory.Build.props#L1-L11)
- [run_win.ps1:16-19](file://run_win.ps1#L16-L19)
...
- **输出目录**：支持 -Output 参数自定义输出目录，默认为 "publish"
...
```powershell
# 基本构建（Release 配置，输出到 publish 目录）
.\run_win.ps1 -Build
...
# 指定自定义输出目录
.\run_win_ps1 -Build -Output "build_output"
...
虽然当前仓库中可能不包含这些已删除的工作流文件，但它们曾经存在于项目中，为 CI/CD 流程提供了重要支撑。这些工作流文件的删除通常意味着：
