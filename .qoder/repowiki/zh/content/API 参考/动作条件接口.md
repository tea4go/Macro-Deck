...
## 更新摘要
**变更内容**
- 更新了 ConditionAction 的序列化逻辑修复，解决了 if-branch 和 else-branch 配置相互覆盖的问题
- 强调了配置持久化机制的改进和错误处理增强
- 新增了序列化测试用例的详细说明
- 补充了配置恢复和向后兼容性的说明
...
**更新** 本版本特别关注了 ConditionAction 的序列化逻辑修复，确保条件动作的配置持久化正确性和条件系统与动作接口的完整性。修复了 if-branch 和 else-branch 配置相互覆盖的关键 bug，并增强了错误处理和配置恢复机制。
...
subgraph "执行层"
CA["ConditionAction<br/>条件判断与动作触发<br/>修复序列化Bug"]
end
subgraph "按钮层"
...
- ConditionAction 条件动作
  ...
  - 触发时会遍历对应动作列表并逐个执行
  - **更新**：具备完善的序列化机制，确保配置持久化正确性，修复了 if-branch 和 else-branch 配置相互覆盖的 bug
- ActionButton 按钮
...
- **更新** 序列化修复：修复了 if-branch 和 else-branch 配置相互覆盖的 bug，确保配置持久化正确性
- 关键枚举：
...
## 结论
IActionConditionItem 为条件项提供了清晰的契约，结合 ConditionItem 的可视化实现与 ConditionAction 的判断执行，构成了完整的条件系统。通过 ActionButton 提供的上下文与 ActionSelectorOnPress 的编辑器集成，开发者可以快速构建复杂的条件动作链。**更新** ConditionAction 的序列化逻辑修复确保了配置持久化的可靠性，特别是修复了 if-branch 和 else-branch 配置相互覆盖的关键 bug，维护了条件系统与动作接口的完整性。新增的测试用例进一步验证了序列化功能的正确性。遵循本文档的实现指南与性能建议，可在保证易用性的同时获得良好的运行效率。
...
- **更新** 配置持久化扩展
  ...
  - 维护向后兼容性，避免破坏现有配置
  - **更新** 确保 if-branch 和 else-branch 配置独立存储，避免相互覆盖
...
### 序列化配置持久化机制
**更新** ConditionAction 的序列化逻辑经过重大修复，确保配置持久化的正确性：
...
- **关键修复**：UpdateConfiguration 方法现在正确地将 if-branch actions 写入 "actions" 键，else-branch actions 写入 "actionsElse" 键，避免相互覆盖
- **向后兼容性**：修复后的序列化逻辑保持与旧配置格式的兼容性
...
- [ConditionActionTests.cs:34-49](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L34-L49)
- [ConditionActionTests.cs:51-66](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L51-L66)

### 测试用例说明
**更新** 新增的序列化测试用例确保了修复的有效性：

- **UpdateConfiguration_KeepsElseBranchSeparateFromIfBranch**：验证 if-branch 和 else-branch 配置不会相互覆盖
- **RoundTrip_KeepsBothBranchesIndependent**：验证序列化和反序列化过程中两个分支的独立性
- **RoundTrip_PreservesConditionFields**：验证条件字段在序列化过程中的正确保留

这些测试用例模拟了 Macro Deck 的实际序列化设置，包括 TypeNameHandling.Auto、NullValueHandling.Ignore 和错误处理回调。

**章节来源**
- [ConditionActionTests.cs:17-88](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L17-L88)