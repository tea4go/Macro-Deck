...
- **回归测试保护**：新增ConditionActionTests.cs确保序列化bug不会再次出现
...
- [ConditionActionTests.cs:10-88](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L10-L88)

### 序列化bug修复详情
**更新** 详细说明了序列化bug的修复过程

- **问题描述**：在GUI中编辑条件动作时，UpdateConfiguration()方法会将if分支动作同时序列化到"actions"和"actionsElse"两个配置键，导致else分支被if分支覆盖
- **修复方案**：
  - 在UpdateConfiguration()方法中，正确地将_actions序列化到"actions"键
  - 正确地将_actionsElse序列化到"actionsElse"键
  - 修正字段名拼写错误：将"contitionMethod"改为"conditionMethod"
- **测试保障**：新增单元测试确保if分支和else分支的配置能够独立持久化，不会相互覆盖

**章节来源**
- [ConditionAction.cs:146-151](file://src/MacroDeck/ActionButton/ConditionAction.cs#L146-L151)
- [ConditionAction.cs:108](file://src/MacroDeck/ActionButton/ConditionAction.cs#L108)
- [ConditionActionTests.cs:34-49](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L34-L49)
- [ConditionActionTests.cs:52-66](file://tests/MacroDeck.Tests/ConditionActionTests.cs#L52-L66)