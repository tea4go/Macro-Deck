...
## 结论
Macro-Deck 的日志与监控体系以 Serilog 为核心，结合运行时级别切换、来源增强、条件输出与 Sentry 白名单过滤，实现了清晰、可控且可诊断的日志生态。调试控制台为开发与用户自助排障提供了直观入口；文件轮转与定期清理保障了长期运行的稳定性。建议在生产环境保持较高最小级别，并谨慎开启调试输出与 Sentry 上报。
